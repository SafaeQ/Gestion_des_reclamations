...
const Tickets = () => {
  ...
  const [queryParams, setQueryParams] = useState<IqueryParams>({
    access_entity: user?.access_entity != null ? user.access_entity : [],
    access_team: user?.access_team != null ? user.access_team : [],
    filter: {},
    pageNumber: 1,
    pageSize: 10,
    read: user?.id != null ? user.id : 0,
    sortField: "updatedAt",
    sortOrder: "desc",
  });
  ...
  useQuery(
    ...
    async () => {
      return await transport
        .post(`tickets/admin/search?body=${value}`, { queryParams })
        .then((res) => res.data);
    },
    {
  ...
  useEffect(() => {
    socket.on("ticket-created", async (ticket: Ticket) => {
      ...
      // });
      toast("You have a new ticket.", {
        data: `New Ticket #${ticket.id}`,
        position: "top-right",
        icon: <BellOutlined style={{ color: "red" }} />,
      });
      await refetch();
      ...
      for (const ticket of data.entities) {
        socket.on(`ticket-updated-${ticket.id}`, async (status: string) => {
          ...
          toast(`Ticket ${ticket.id} is ${status}`, {
            data: `Ticket ${ticket.id} is ${status}`,
            position: "top-right",
          });
          updateTicket();
          await refetch();
    ...
    return () => {
      socket.removeListener("ticket-created");
      socket.removeListener("ticket-forwarded");
  ...
  function renderStatus(status: TICKET_STATUS) {
    switch (status) {
      case TICKET_STATUS.In_Progress:
        return <Tag color="processing">{TICKET_STATUS.In_Progress}</Tag>;
      case TICKET_STATUS.Open:
        return <Tag color="red">{TICKET_STATUS.Open}</Tag>;
      case TICKET_STATUS.Reopened:
        return <Tag color="warning">{TICKET_STATUS.Reopened}</Tag>;
      case TICKET_STATUS.Resolved:
        return <Tag color="green">{TICKET_STATUS.Resolved}</Tag>;
      case TICKET_STATUS.Closed:
        return <Tag color="purple">{TICKET_STATUS.Closed}</Tag>;
    }
  }
  ...
  const columns: Array<ColumnProps<Ticket>> = [
    {
      ...
      render: (_, record) => (
        <Link to={`/tickets/${record.id}/edit`}>{record.id}</Link>
      ),
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Badge offset={[0, 0]} status="success" count={record.unread}>
            <Button
              shape="circle"
    ...
    },
    {
      ...
      children: (
        <Table
          ...
          columns={columns}
          dataSource={
            value.length !== 0 ? filteredData : filterdTicketsReopened
          }
        />
      ),
    },
        ...
        extra={[
          <Space key={0}>
            ...
            <Popconfirm
              ...
              onConfirm={(e) => confirmDelete(selectedRowKeys)}
              onCancel={cancel}
              okText="Yes"
              cancelText="No"
            >

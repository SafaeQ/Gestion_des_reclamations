...
const HolidayHome = () => {
  ...
  const { mutate: mutateStatus } = useMutation(
    ...
    {
      onSuccess: async () => {
        void message.success("update Status is done");
        void refetch();
      },
      onError: async () => {
        void message.error("Error Updating");
  ...
  const deleteMutation = useMutation(
    async ({ id }: { id: number }) => {
      return await transport.delete(`/holidays/delete/${id}`);
    },
    {
      onSuccess: async () => {
        void message.success("holiday deleted Succesfully");
        void refetch();
      },
      onError: async () => {
        void message.error("Error deleting ");
      },
    }
  );
  ...
  const columns: Array<ColumnProps<Holiday>> = [
    ...
    },
    {
      ...
      key: "user",
      dataIndex: "user",
      render: (_, record) => record.user.solde,
    },
    {
      title: "Date",
      key: "createAt",
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <>
            <Tooltip title="Approve" color="geekblue">
              <Popconfirm
          ...
          <Tooltip title="Delete" color="geekblue">
            <Popconfirm
              key={4}
              title={"Are you sure you want to delete this Request holiday?"}
              onConfirm={() => {
                setLoading({ key: record.id, load: true, type: "DELETE" });
